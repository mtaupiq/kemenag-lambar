  ...
  <div class="container-fluid">
    <div class="card">
      <div class="card-header">
        <h3 class="card-title">Update Hero Slide</h3>
      </div>
              
      <!-- /.card-header -->
      <div class="card-body">
        <form action="{{ route ('slider.update', $slider->title) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PATCH')
            ...
            <div class="form-group row">
                ...
                <div class="col-sm-10">
                <input type="text" class="form-control @error('caption') is-invalid @enderror" id="caption" name="caption" value="{{ $slider->caption }}">
                @error('caption')
                  <p class="text-danger">{{ $message }}</p>
                  @enderror
                </div>
            ...
            <div class="form-group row">
                ...
                <div class="col-sm-10">
                <input type="text" class="form-control @error('link') is-invalid @enderror" id="link" name="link" value="{{ $slider->link }}">
                @error('link')
                  <p class="text-danger">{{ $message }}</p>
                  @enderror
                </div>
