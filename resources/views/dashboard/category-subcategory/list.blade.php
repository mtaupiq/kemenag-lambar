                    ...
                    <div class="col-md-6 dd" id="nestable-wrapper">
                            ...
                            @foreach($categories as $k => $category)
                                <li class="dd-item list-group-item" data-id="{{ $category['category_id'] }}" >
                                    ...
                                    <div class="dd-option-handle">
                                        <a class="btn btn-warning btn-xs" href="{{ route('category-subcategory.edit', ['category_id' => $category['category_id'] ]) }}">
                                            <i class="fas fa-edit"></i> 
                                        </a>
                                        <a class="btn btn-danger btn-xs" href="{{ route('category-subcategory.remove', ['category_id' => $category['category_id'] ]) }}">
                                    ...
                                    @if(!empty($category->categories))
                                        @include('dashboard.category-subcategory.child-includes.child-category-view', [ 'category' => $category])
                                    @endif
                                </li>
                            @endforeach
                        </ol>
                        <div class="row">
                            <form action="{{ route('category-subcategory.save-nested-categories') }}" method="post">
                                ...
                                <button type="submit" class="btn btn-success" style="margin-top: 15px;" >Save Change</button>
                            </form>
                        </div>
                            </div>
                        </div>
