  ...
  <div class="container-fluid">
    <div class="card">
      <div class="card-header">
        <h3 class="card-title">Update Layanan</h3>
      </div>
              
      <!-- /.card-header -->
      <div class="card-body">
        <form action="{{ route ('service.update', $service->title) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PATCH')
            ...
            <div class="form-group row">
                ...
                <div class="col-sm-10">
                <input type="text" class="form-control @error('caption') is-invalid @enderror" id="caption" name="caption" value="{{ $service->caption }}">
                @error('caption')
                  <p class="text-danger">{{ $message }}</p>
                  @enderror
                </div>
            ...
            <div class="form-group row">
                ...
                <div class="col-sm-10">
                <input type="text" class="form-control @error('link') is-invalid @enderror" id="link" name="link" value="{{ $service->link }}">
                @error('link')
                  <p class="text-danger">{{ $message }}</p>
                  @enderror
                </div>
